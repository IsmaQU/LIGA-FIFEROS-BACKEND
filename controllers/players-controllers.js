...
var clientNowDate = moment().format();
var TimezoneMadrid = moment.tz("Europe/Madrid").format();
var madridTimezonemonth = TimezoneMadrid.substring(5,7);
var madridTimezoneyear = TimezoneMadrid.substring(0,4);
var initDateSummerTransfer = moment.tz(`${madridTimezoneyear}-${madridTimezonemonth}-15 22:30`, "Europe/Madrid");
var endDateClausulazos = moment.tz(`${madridTimezoneyear}-${madridTimezonemonth}-17 10:30`, "Europe/Madrid");
var endDateSummerTransfer = moment.tz(`${madridTimezoneyear}-${madridTimezonemonth}-18 22:30`, "Europe/Madrid");
var initDateWinterTransfer = moment.tz(`${madridTimezoneyear}-${madridTimezonemonth}-01 22:30`, "Europe/Madrid");
var endDateWinterTransfer = moment.tz(`${madridTimezoneyear}-${madridTimezonemonth}-03 22:30`, "Europe/Madrid");
var clientInitDateSummerTransfer = initDateSummerTransfer.clone().tz(clientTimeZone).format();
var clientEndDateClausulazos = endDateClausulazos.clone().tz(clientTimeZone).format();
var clientEndDateSummerTransfer = endDateSummerTransfer.clone().tz(clientTimeZone).format();
var clientInitDateWinterTransfer = initDateWinterTransfer.clone().tz(clientTimeZone).format();
var clientEndDateWinterTransfer = endDateWinterTransfer.clone().tz(clientTimeZone).format();
...
const updatePlayer = async (req, res, next) => {
  ...
  const debt = /* Number(clausula) - Number(player.clausula) */ Number(inputQuantity) + Number(sum);
...
const resetPlayerClause = async (req, res, next) => {
  const { clausula } = req.body;
  const playerId = req.params.pid;
...
const deletePlayer = async (req, res, next) => {
  ...
  /* if (
    ...
    (actionType === "Clausulazo" &&
      clientNowDate >= clientEndDateClausulazos &&
      user.equipo !== "Admin") ||
    (actionType === "Clausulazo" &&
      clientNowDate < clientInitDateSummerTransfer &&
      user.equipo !== "Admin")
  ) {
    const error = new HttpError(
      "Operación cancelada, solo se puede pagar cláusulas de rescisión del 15 de cada mes a las 22:30 horas hasta el 17 de cada mes a las 10:30 horas",
      404
    );
    return next(error);
  } */
  ...
  /* if (
    ...
    (user.equipo !== "Admin" && player.title === "Prueba1") ||
    (actionType !== "Clausulazo" &&
      clientNowDate >= clientEndDateSummerTransfer &&
      clientNowDate < clientInitDateWinterTransfer &&
      user.equipo !== "Admin") ||
    (actionType !== "Clausulazo" &&
      clientNowDate >= clientEndDateWinterTransfer &&
      clientNowDate < clientInitDateSummerTransfer &&
      user.equipo !== "Admin")
  ) {
    const error = new HttpError(
...
exports.resetPlayerClause = resetPlayerClause;
